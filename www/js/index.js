...
//SMACKTALK Data
var smackTalkMsg = ["You're Going Down!",
                    "I'm The Best!",
                    ...
                    "No Time For Loser!",
                    "Cuz We Are The Champion Of The World!",
                    "BOOYAH!",
                    "SUCKA!",
                    "Bet'chu I'm Gonna Win",
                    "HA! TAKE THAT!",
                    "Go Big Or Go Home!",
                    "Big Boy's Leauge Now!",
                    "\'Sup Foo?",
                    ...
                    "Never Gonna Go Around And Hurt You!",
                    "BowChikaWowWow~",
                    ...
                    "Got 99 Problem But A Lose Ain't One!",
                    "Got The Moves, Got The Groves~"
];
var smackTalkSender = ["Stacy Rampersad",
        "Carlos Batista",
        "Teresa Harris",
        "Alanna Glass",
        "Roy Cheung",
...
var generateSmackTalk = function(){
	return "SMACKTALK:" + smackTalkSender[Math.round(Math.random()*(smackTalkSender.length - 1))] + ", \"" + smackTalkMsg[Math.round(Math.random()*(smackTalkMsg.length - 1))] + "\"";
};

console.log(generateSmackTalk());

var returnQuery = function(url) {
    console.log('http://query.yahooapis.com/v1/public/yql?q=' + encodeURIComponent('select * from xml where url="' + endpoint + url + access_token + '"') + '&format=xml&callback=?');
...
var app = {
    ...
    receivedEvent: function(id) {
        ...
        GalaxyGear.onConnect(function(e) {
        	GalaxyGear.sendData(e.handle, "CurrentWeek" + currentweek);
            ...
            GalaxyGear.onDataReceived(e.handle, function(e) {
                console.log("Data received - handle: " + e.handle + " data: " + e.data);
                //GalaxyGear.sendData(e.handle, "DANNY HA has recieved the data!");

                var temp = e.data;
                //check if recieved data is SMACKTALK string
                if (temp.match(/^SMACKTALK/) != null) {
                    alert(temp.slice(9));
                } else if (temp.match('FETCHMATCHUP') != null) {
                    GalaxyGear.sendData(e.handle, weeklyMatchup);
                }
            });
            ...
            $('#send-message-2').on('click', function() {
                GalaxyGear.sendData(e.handle, generateSmackTalk());
            });

            $('#send-message-3').on('click', function() {
                GalaxyGear.sendData(e.handle, weeklyMatchup);
            });
            
            $('#send-message-4').on('click', function() {
                GalaxyGear.sendData(e.handle, "Reminder Alert!, Set your lineup for Week 12");
            });
            
            $('#send-message-5').on('click', function() {
                GalaxyGear.sendData(e.handle, "Giovani Bernard, OUT with torn ACL");
            });
            
            $('#send-message-6').on('click', function() {
                GalaxyGear.sendData(e.handle, "Demarco Murray, rushed 45 yards for a TD");
            });
        });
...
function onNotification(e) {
    ...
    // Send Notification String To Gear
    GalaxyGear.sendData(channelID, e.payload.message);
